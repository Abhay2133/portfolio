    ...
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {projects.map((project) => (
          <div key={project.title} className="group relative flex flex-col p-3 rounded-2xl border border-neutral-100 shadow-sm hover:border-neutral-200 hover:shadow-md hover:-translate-y-0.5 transition-all gap-4 bg-white/50 backdrop-blur-sm">
            <a href={project.link} className="w-full h-40 shrink-0 rounded-xl overflow-hidden bg-neutral-100 block relative">
            ...
            <div className="flex flex-col grow px-1">
              <div>
                <a href={project.link} className="inline-flex items-center gap-1 font-medium text-neutral-900 group-hover:text-blue-600 transition-colors text-base tracking-tight">
                  {project.title}
                  <ArrowUpRight className="w-3.5 h-3.5 text-neutral-400 group-hover:text-blue-600 transition-colors" />
                </a>
              </div>
              <p className="text-sm text-neutral-600 leading-relaxed line-clamp-2 mt-1 shrink-0">
                {project.description}
              </p>
              <div className="text-xs font-mono text-neutral-500 mt-auto pt-4 flex flex-wrap gap-x-3 gap-y-1">
                {project.tech.map((t, i) => (
                  <span key={t} className="flex items-center">
                    {t}{i < project.tech.length - 1 && <span className="ml-3 w-1 h-1 bg-neutral-300 rounded-full" />}
                  </span>
                ))}
              </div>
            </div>
